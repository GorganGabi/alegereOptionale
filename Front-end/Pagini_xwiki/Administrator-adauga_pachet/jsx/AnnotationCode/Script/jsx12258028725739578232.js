...
XWiki.Annotation = Class.create({
  ...
  toggleSettingsPanel : function(event) {
    ...
    if (!this.settingsPanel) {
      new Ajax.Request('http://localhost:8181/xwiki/bin/view/AnnotationCode/Settings?xpage=plain', {
  ...
  addEditListener : function(item, id, container, inBubble) {
    item.observe('click', function(event) {
      ...
      if (item.disabled) {
        ...
        require(['xwiki-meta'], function (xm) {
          new Ajax.Request('http://localhost:8181/xwiki/bin/view/AnnotationCode/EditForm', {
            parameters: {
              'xpage' : 'plain',
  ...
  fetchAndShowAnnotationDetails : function(annotationId, container) {
    require(['xwiki-meta'], function (xm) {
      new Ajax.Request('http://localhost:8181/xwiki/bin/view/AnnotationCode/DisplayForm', {
        parameters: {
          'id' : annotationId,
          'xpage' : 'plain',
          'wiki' : XWiki.currentWiki,
  ...
  onAddAnnotationShortcut : function() {
    ...
    if (!selectionText) {
      ...
      require(['xwiki-meta'], function (xm) {
        ...
        new Ajax.Request('http://localhost:8181/xwiki/bin/view/AnnotationCode/CreateForm', {
          parameters: {
            'xpage' : 'plain',
